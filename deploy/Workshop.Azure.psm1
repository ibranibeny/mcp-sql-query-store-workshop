...
"@
            $result = Invoke-AzVMRunCommand -ResourceGroupName $ResourceGroupName -VMName $VmName `
                -CommandId RunPowerShellScript -ScriptString $command -ErrorAction Stop
            # Take only StdOut. A non-empty StdErr (for example when the bootstrap failed before
            # writing the readiness file) must not be concatenated into the compressed payload,
            # which would otherwise surface as a misleading base-64 decoding error.
            $encoded = (@($result.Value | Where-Object { $_.Code -like '*StdOut*' } | ForEach-Object Message) -join '').Trim()
            $standardError = (@($result.Value | Where-Object { $_.Code -like '*StdErr*' } | ForEach-Object Message) -join '').Trim()
            if ([string]::IsNullOrWhiteSpace($encoded)) {
                $detail = if ([string]::IsNullOrWhiteSpace($standardError)) { 'no output was returned' } else { "the guest reported: $standardError" }
                throw "Readiness could not be read from '$Path'; $detail. The bootstrap most likely failed before writing its readiness record."
            }
            try { $compressed = [Convert]::FromBase64String($encoded) }
            catch { throw "Readiness read from '$Path' did not return valid compressed content; the bootstrap most likely failed before writing its readiness record." }
...
function Test-WorkshopReadiness {
    ...
    # Each record is normally a single 'publisher.name@version' token. Tolerate a legacy shape where
    # multiple tokens were captured into one whitespace-joined string by splitting on whitespace first,
    # then take the identifier ahead of the '@' version separator.
    $extensionIds = @(
        $extensionRecords |
    ...
    # GitHub Copilot / Copilot Chat ship as built-in extensions on current VS Code builds, so they
    # are not reported by --list-extensions. Count any recorded built-in extensions toward the
    # required set alongside the marketplace-installed extensions.
    $builtInExtensionIds = @(Get-ReadinessValue $AdminReadiness 'Tools.BuiltInExtensions')
    $allExtensionIds = @($extensionIds + $builtInExtensionIds)
    # Installing ms-mssql.mssql automatically pulls dependency extensions (data-workspace,
    # sql-bindings, sql-database-projects, vscode-dotnet-runtime), so the observed extension set is a
    # superset of the required set. Assert that every required extension is present instead of
    # demanding an exact set, which would break whenever the marketplace dependency graph changes.
    $requiredExtensionsPresent = @($expectedExtensions | Where-Object { $_ -notin $allExtensionIds }).Count -eq 0
    ...
    foreach ($check in @(
        ...
        @{ Name = 'SQL VM exact identity and private boundary'; Passed = (Get-ReadinessValue $SqlReadiness 'Vm.Name') -ceq 'vm-mcpsql-sql' -and (Get-ReadinessValue $SqlReadiness 'Vm.Size') -ceq 'Standard_E8s_v5' -and ([string](Get-ReadinessValue $SqlReadiness 'Vm.Location')).ToLowerInvariant() -ceq 'indonesiacentral' -and (Get-ReadinessValue $SqlReadiness 'Vm.PublicIp') -eq $false }
        ...
        @{ Name = 'Administration VM exact identity and security'; Passed = (Get-ReadinessValue $AdminReadiness 'Vm.Name') -ceq 'vm-mcpsql-admin' -and (Get-ReadinessValue $AdminReadiness 'Vm.Size') -ceq 'Standard_D4s_v5' -and ([string](Get-ReadinessValue $AdminReadiness 'Vm.Location')).ToLowerInvariant() -ceq 'indonesiacentral' -and (Get-ReadinessValue $AdminReadiness 'Vm.AdminPublicIpBoundaryObserved') -eq $true -and (Get-ReadinessValue $AdminReadiness 'Vm.PublicIpCount') -le 1 -and (Get-ReadinessValue $AdminReadiness 'Vm.SecureBoot') -eq $true -and (Get-ReadinessValue $AdminReadiness 'Vm.Tpm') -eq $true -and ([string](Get-ReadinessValue $AdminReadiness 'Vm.Os')) -match 'Windows 11 Enterprise' -and [int](Get-ReadinessValue $AdminReadiness 'Vm.Build') -ge 26100 -and (Get-ReadinessValue $AdminReadiness 'Vm.WindowsClientLicenseAttested') -eq $true -and $activation -in @('Licensed', 'ObservedUnknown') }
        @{ Name = 'Administration exact tools and extensions'; Passed = (Test-ExactSet -Actual $packageIds -Expected $expectedPackages) -and $packageVersionsObserved -and $requiredExtensionsPresent -and $dabVersionValid }
